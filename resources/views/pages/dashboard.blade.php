<x-layouts.app title="لوحة التحكم">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-1">
                        {{ $attendances->where('date', now()->toDateString())->count() }}</p>
                    <p class="text-xs text-gray-500 mt-2">
                        {{ $attendances->where('date', now()->toDateString())->where('status', 'absent')->count() }}
                        غائب اليوم
                    </p>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-orange-600 mt-1">
                        {{ number_format($outstandingBalance ?? 12500, 2) }}</p>
                    <p class="text-xs text-gray-500 mt-2">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <a href="{{ route('attendance.create') ?? '#' }}"
            class="bg-white rounded-xl border border-gray-200 p-4 hover:border-primary-300 hover:shadow-md transition-all group">
            <div
                class="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
        ...
        <a href="{{ route('workers.create') ?? '#' }}"
            class="bg-white rounded-xl border border-gray-200 p-4 hover:border-primary-300 hover:shadow-md transition-all group">
            <div
                class="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
        ...
        <a href="{{ route('payments.create') ?? '#' }}"
            class="bg-white rounded-xl border border-gray-200 p-4 hover:border-primary-300 hover:shadow-md transition-all group">
            <div
                class="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
        ...
        <a href="{{ route('reports.monthly') ?? '#' }}"
            class="bg-white rounded-xl border border-gray-200 p-4 hover:border-primary-300 hover:shadow-md transition-all group">
            <div
                class="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div class="divide-y divide-gray-100">
                @forelse($attendances->where('date', now()->toDateString())->take(5) as $att)
                    <div class="px-6 py-3 flex items-center justify-between hover:bg-gray-50">
                        <div class="flex items-center">
                            <div class="w-8 h-8 bg-primary-100 rounded-full flex items-center justify-center ml-3">
                                <span
                                    class="text-xs font-semibold text-primary-700">{{ mb_substr($att->worker->name, 0, 1) }}</span>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-900">{{ $att->worker->name }}</p>
                                <p class="text-xs text-gray-500">{{ $att->check_in_time ?? '—' }} -
                                    {{ $att->check_out_time ?? '—' }}</p>
                            </div>
                        </div>
                ...
                @empty
                    <div class="px-6 py-8 text-center">
                        <p class="text-gray-500">لا يوجد حضور مسجل اليوم</p>
                    </div>
                @endforelse
            </div>
        </div>
            ...
            <div class="divide-y divide-gray-100">
                ...
                @foreach ($recentPayments as $payment)
                    <div class="px-6 py-3 flex items-center justify-between hover:bg-gray-50">
                        ...
                        <div class="text-left">
                            <p class="text-sm font-bold text-green-600">{{ number_format($payment['amount'], 2) }} ج.م
                            </p>
