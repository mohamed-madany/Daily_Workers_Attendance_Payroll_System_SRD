<x-layouts.app title="السجل اليومي">
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-gray-50 border-b border-gray-200">
            <div class="bg-white rounded-lg p-4 border border-gray-200">
                ...
                <p class="text-xl font-bold text-gray-900">{{ number_format($ledgers->sum('gross_earned_amount'), 2) }}
                    ج.م</p>
            </div>
            <div class="bg-white rounded-lg p-4 border border-gray-200">
                <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">إجمالي الخصومات</p>
                <p class="text-xl font-bold text-red-600">{{ number_format($ledgers->sum('deductions_amount'), 2) }} ج.م
                </p>
            </div>
            <div class="bg-white rounded-lg p-4 border border-gray-200">
                <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">صافي الأرباح</p>
                <p class="text-xl font-bold text-green-600">{{ number_format($ledgers->sum('net_earned_amount'), 2) }}
                    ج.م</p>
            </div>
        </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($ledgers as $ledger)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div
                                        ...
                                        <span
                                            class="text-xs font-semibold text-green-700">{{ mb_substr($ledger->worker->name ?? 'ع', 0, 1) }}</span>
                                    </div>
                                    <span
                                        class="text-sm font-medium text-gray-900">{{ $ledger->worker->name ?? 'غير معروف' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-gray-900">{{ $ledger->date }}</span>
                            </td>
                            <td class="px-6 py-4 text-left">
                                <span
                                    class="text-sm font-medium text-gray-900">{{ number_format($ledger->gross_earned_amount, 2) }}
                                    ج.م</span>
                            </td>
                            <td class="px-6 py-4 text-left">
                                @if ($ledger->deductions_amount > 0)
                                    <span
                                        class="text-sm font-medium text-red-600">-{{ number_format($ledger->deductions_amount, 2) }}
                                        ج.م</span>
                                @else
                            ...
                            <td class="px-6 py-4 text-left">
                                <span
                                    class="text-sm font-bold text-green-600">{{ number_format($ledger->net_earned_amount, 2) }}
                                    ج.م</span>
                            </td>
                        </tr>
                    @empty
                ...
                @if (count($ledgers) > 0)
                    <tfoot class="bg-gray-50 border-t-2 border-gray-300">
                        <tr>
                            ...
                            <td class="px-6 py-4 text-left">
                                <span
                                    class="text-sm font-bold text-gray-900">{{ number_format($ledgers->sum('gross_earned_amount'), 2) }}
                                    ج.م</span>
                            </td>
                            <td class="px-6 py-4 text-left">
                                <span
                                    class="text-sm font-bold text-red-600">-{{ number_format($ledgers->sum('deductions_amount'), 2) }}
                                    ج.م</span>
                            </td>
                            <td class="px-6 py-4 text-left">
                                <span
                                    class="text-sm font-bold text-green-600">{{ number_format($ledgers->sum('net_earned_amount'), 2) }}
                                    ج.م</span>
                            </td>
                        </tr>
                    </tfoot>
    ...
    <div class="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-xl">
        <div class="flex items-start">
            <svg class="w-5 h-5 text-blue-600 mt-0.5 ml-3 flex-shrink-0" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
